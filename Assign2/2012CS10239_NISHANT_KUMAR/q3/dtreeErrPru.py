...
def growTree(inpd,curNode,attrset):
	...
	# use error as the metric to split on
	spAttr = chooseBestAttrError(data0+data1,attrset) # spAttr is in [1..maxattr]
...
def findValAcc(root):
	...
	validCorrect = 0
	for ex in vld:
		prediction = predictSingle(ex[1:],root)
		if not((prediction==0) or (prediction==1)):
			print "predictSingle not working !!"
			exit(1)
		if (prediction == ex[0]): validCorrect += 1

	return (1.0*validCorrect)/len(vld)

def pruneStep(curNode,root,oriValAcc):
	maxi,par,child = oriValAcc,None,None
	if (curNode.leftNode):
		temp = curNode.leftNode
		curNode.leftNode = None
		a1 = findValAcc(root)

		curNode.leftNode = temp
		[a2,apar,achnum] = pruneStep(curNode.leftNode,root,oriValAcc)
		if (a1>a2) and (a1>=maxi):
			maxi = a1
			par = curNode
			child = 0
		elif (a2>=a1) and (a2>=maxi):
			maxi = a2
			par = apar
			child = achnum

	if (curNode.midNode):
		temp = curNode.midNode
		curNode.midNode = None
		a1 = findValAcc(root)

		curNode.midNode = temp
		[a2,apar,achnum] = pruneStep(curNode.midNode,root,oriValAcc)
		if (a1>a2) and (a1>=maxi):
			maxi = a1
			par = curNode
			child = 1
	...
	if (curNode.rightNode):
		temp = curNode.rightNode
		curNode.rightNode = None
		a1 = findValAcc(root)
		
		curNode.rightNode = temp
		[a2,apar,achnum] = pruneStep(curNode.rightNode,root,oriValAcc)
		if (a1>a2) and (a1>=maxi):
			maxi = a1
			par = curNode
			child = 2
			...
			child = achnum
	if (child!=None):
		# means child is not none - and hence has been updatad
		return [maxi,par,child]
	else:
		return [-1,None,None]

def prune():
	global classifier,prunNumNodes,prunTrainAcc,prunValAcc,prunTestAcc
	curacc = findValAcc(classifier)
	classifierOri = classifier
	c1 = classifierOri
	c2 = classifierOri
	[newacc,par,child] = pruneStep(c1,c2,curacc)
	findAccuracy(prunNumNodes,prunTrainAcc,prunValAcc,prunTestAcc,classifierOri)
	while(newacc>=curacc):
		if (child==0):
			par.leftNode = None
		elif (child==1):
			...
			print "Some errror in prune"
			exit(1)
		if (par.leftNode==None) and (par.midNode==None) and (par.rightNode==None):
			# means this parent's all 3 children have been pruned
			par.splitAttr = None
		curacc = newacc
		c1 = classifierOri
		c2 = classifierOri
...
prune()
print 'The final number of nodes obtained after pruning = ', classifier.findNumNodes()
...
fig, ax = plt.subplots()
ax.plot(prunNumNodes, prunTrainAcc,'--', label='PrunTrainData accuracy')
ax.plot(prunNumNodes, prunValAcc, marker='2', ls=':', label='PrunValData accuracy')
ax.plot(prunNumNodes, prunTestAcc, marker='3' , ls='--',label='PrunTestData accuracy')
